...
- 프로필 위저드에서는 먼저 키캡 표기를 `Mac` 또는 `Windows / VDI`로 선택합니다
- 다음 단계에서 스페이스바 왼쪽 modifier를 실제로 눌러 현재 입력 순서를 감지합니다
- 현재 입력이 3개면 3키, 4개면 4키로 자동 판단합니다
- `Mac 로컬`과 `VDI`에서 원하는 배치는 각각 왼쪽부터 순서대로 선택합니다
- `VDI` 단계는 비어 있는 상태로 시작하며, 같은 배열을 원할 때만 `Mac 로컬 배치 복사`를 사용합니다
- 목표 배열 선택 단계에서는 `Win (Cmd)`, `Alt (Opt)`처럼 실제 macOS 입력명까지 같이 표시됩니다
