...
> **CLEAN-1 + SCROLL-1 = 사용자 commit (2026-05-31, v1.6.0 publish 후 재확인)** — "최종적으로 윈맥키에 들어가야 한다" 명시. 구현 시점은 P9/P10 핫픽스 + 디자인 표준화([[project_winmackey_design_standards]]) 이후 합의.
> 그 외 항목은 명세만 확정, 우선순위·UX는 별도 합의.

### CLEAN-1 · 키보드 클리닝 시스템 (Keyboard Cleaning Mode)
| 항목 | 내용 |
|---|---|
| **목적** | 키보드를 닦는 동안 **모든 키 입력을 차단** — 닦다가 키가 눌려 글자 입력·단축키·삭제 등이 일어나는 것을 방지. |
| **동기 / 경쟁 대안** | macOS 빌트인에 "키보드만 잠금" 기능 없음. 사용자가 `KeyboardCleanTool` ($9.99) 같은 별도 앱을 깔아 쓰는 상황 — **WinMacKey 가 이미 손쉬운 사용 권한 + CGEventTap 인프라 보유라 무리 없이 통합 가능.** 단일 설치로 끝. |
...
| **상태** | 🟡 **사용자 commit (2026-05-31), 구현 대기** |
...
| **동기 / 경쟁 대안** | 현재 사용자는 **Logitech Options (LogiOptions+ / LogiTune)** 를 깔아서 이 분리를 구현 중. 문제점: ① **로지텍 마우스 전용** (Logitech HID 만 인식, 타사 마우스 X) ② Logi 데몬 백그라운드 상주 + 자동 업데이트 + 텔레메트리 ③ macOS 메이저 업그레이드마다 호환성 끊김 이력. **WinMacKey 가 통합하면 — ① 브랜드 무관(모든 USB/BT 마우스) ② 데몬 1개(WinMacKey) ③ 단일 설치/공증.** OSS 대안 `Mos` / `LinearMouse` / `Scroll Reverser` 도 별도 설치 필요. |
...
| **켜기/끄기** | 메뉴바 / 설정 토글. (선택) 장치별 화이트리스트 — "이 마우스만 반전". 기본 = ON (Windows 출신 사용자 정체성). |
...
| **안전장치** | ① 토글 OFF 시 즉시 원상복구(이벤트 통과만). ② momentum/관성 스크롤은 트랙패드만, 마우스는 step 단위 그대로. ③ 줌(⌘+휠) 같은 modifier 조합은 부호 반전 영향 검토 — 필요 시 제외. ④ 게임 등 raw input 받는 앱은 화이트리스트로 제외. |
| **상태** | 🟡 **사용자 commit (2026-05-31), 구현 대기 — Logi Options 대체 가치 명확** |

### 통합 시점 합의 사항
- **선행 조건**: P9/P10 핫픽스 (사용자 보고 한영 간헐 / Ghostty ESC) 진단·해결, [[14-WinMacKey-v1.6.0-Release]] 의 감사 Top 5 처리.
- **디자인 일관성**: [[project_winmackey_design_standards]] (`docs/design/`) 의 UI/UX 기준에 맞춰 메뉴바·설정 UI 확장. 두 기능이 메뉴바 권한 섹션과 충돌 없이 들어갈 자리 합의 필요.
- **유료화 후보**: SCROLL-1 은 *Windows 사용성을 Mac에서* 정체성의 핵심 차별점 — Pro 게이팅 후보로도 검토 (현재 Pro = 디바이스별 자동 전환만, 확장 가능성). CLEAN-1 은 단순 유틸이라 무료 유지.
