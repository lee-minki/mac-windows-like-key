...
- 외장 키보드 프로필을 쓸 경우 **3화면 표 위자드**에서 매핑을 잡습니다:
  - **시작 화면**: 프로필 이름 입력 + `"키 배치도 바꾸기"` 선택
  - **키보드 형태**: 실제 키캡 순서를 선택합니다. Keys-To-Go 2처럼 `Ctrl · Fn · Opt · Cmd`면 **포터블 Mac 4키**를 선택합니다.
  - **매핑 표**: 포터블 4키의 Mac 추천값은 `Cmd · Fn · Opt · Ctrl`, VDI 자동값은 `Ctrl · Win · Win · Alt`입니다.
