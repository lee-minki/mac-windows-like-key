...
- 외장 키보드 프로필을 쓸 경우 **3화면 표 위자드** (v1.6.0+, 캡처 없음) 에서 매핑을 잡습니다:
  - **시작 화면**: 프로필 이름 입력 + `"키 배치도 바꾸기"` 선택
  - **매핑 표**: `Fn · Ctrl · Opt · Cmd` 4행 × `Mac 에서` / `VDI 에서` 2열. VDI 열에는 `Ctrl · Win · Alt` 선택지가 뜸. 표 상단 `Windows 키보드` 토글로 키캡 표기를 `Cmd/Opt` → `Win/Alt` 로 전환 (cosmetic). `"Windows 감각"` 버튼 = Mac 에서 Cmd↔Ctrl 스왑 1클릭.
  - **확인·저장**: 매핑 요약 확인 후 저장하고 적용.
- macOS 가 모든 키보드를 같은 키코드로 보고하므로 **키보드를 누를 필요 없음** — 키캡 인쇄가 `Win`/`Alt` 든 `Cmd`/`Opt` 든 동일하게 동작합니다.
