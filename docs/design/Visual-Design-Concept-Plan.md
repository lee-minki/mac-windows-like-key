...
- 페이월(Pro) 화면 **한정**으로 Raycast식 다크 프리미엄 무드(딥다크+은은한 글로우) 허용 — 단, 시스템 다크모드와 충돌 없게.
...
- **인트로 비주얼 = 자체 에셋 `05_permission_onboarding`**(brand 민트/틸, 키보드→방패→⌘→한/A 흐름). 단 §2-5 주의대로 "그림"임을 명시(옅은 박스+섀도우, 스케일 다운) + 실제 동작 버튼 분리 + **다크모드 변형 별도**. 앱 아이덴티티는 아이콘 A(`⌘`+`한⇄A`).
- **단, 각 권한 스텝의 실제 안내는 일러스트가 아니라 §2-3 실 스크린샷 미러링**(가짜 시스템UI 금지 — 팀 노트·Gemini 공통 경고).
- **Step 1 — Accessibility**: 파란 테마, `MiniSystemSettingsIllustration`(펄스 빨간 원). "Touch ID/암호를 물어볼 수 있어요(정상)" 미리 안내.
- **Step 2 — Input Monitoring**: **의도적으로 다른 색(예: 보라/녹색 테마) + "STEP 2 / 두 번째 권한" 큰 라벨**로 1번과 시각 구분(착각 방지 ★). 형광펜 하이라이트 미러링.
...
### ③ Pro 페이월
- 트리거: 수동 프로파일 전환 N회 / 잠긴 자동전환 클릭 / 10일+ 사용. 나그 모달 ❌, 맥락 시점 ✅.
- 레이아웃: Raycast식 — 이득 3–5개 benefit-led, `PlanCard`(가격 대형), pill CTA. **Gumroad 브라우저 핸드오프 기대치 안내 + 브랜딩 일치**.
- 자동전환 기능은 평소 `ProBadge` 달고 visible-but-locked로 노출.
