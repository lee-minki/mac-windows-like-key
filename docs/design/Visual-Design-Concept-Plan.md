...
> **수집 이미지**: [`references/`](./references/) (21장)
...
- **다크모드 = Pro 전용 기능** (2026-05-31 결정). **Free = 라이트 모드 고정**, Pro = 라이트+다크(+테마). Raycast가 Custom Themes를 Pro로 가둔 선례와 동일.
- 따라서 **페이월 화면의 다크 프리미엄 무드 = "Pro로 가면 이런 다크"의 미리보기**로 정당화됨(이전 #3 고민 해소).
- ⚠️ 고려사항: macOS 앱은 보통 시스템 외관을 자동 추종 → **시스템이 다크인데 Free 앱만 라이트로 고정**되면 "안 따라온다"는 위화감 가능. (a) 그래도 다크 전체를 Pro로 잠글지, (b) 시스템 다크 추종은 Free에 주고 **커스텀 테마/액센트만 Pro**로 할지는 구현 전 최종 확인 필요. → `docs/private/PRO_TIER_GATING_PLAN.md`(현재 "자동전환만 Pro") 갱신 대상.
...
### ① 온보딩 위저드 (3스텝, 진행률 바, 스킵·재실행 가능)
- 좌측 스텝 인디케이터(1·2·3) + 우측 `PrimingCard`. HIG: 빠르고·스킵 가능·재노출 금지.
...
- **온보딩은 Free 단계 → 항상 라이트 모드**(다크=Pro). 온보딩 일러스트 다크 변형 불필요(설정/페이월 등 Pro 화면만 다크 대응).
- **Step 1 — Accessibility**: `MiniSystemSettingsIllustration`(펄스 강조). "Touch ID/암호를 물어볼 수 있어요(정상)" 미리 안내.
- **Step 2 — Input Monitoring**: **"STEP 2 / 두 번째 권한 · 다른 화면" 큰 라벨 + 다른 일러스트 콘텐츠**로 1번과 구분(착각 방지 ★). ※ 구분 수단은 라벨·콘텐츠 위주 — 그린/블루는 의미색(브랜드·시스템)이라 스텝 구분색으로 남용 금지(#2 논의 반영, 색 최종은 추후).
- **Step 3 — 완료 & 재실행**: `[activated enabled]` 스포트라이트식 상태 확인 + 녹색 ✅, "Quit & Reopen" Primary 버튼.
- **완료 직후 가치 행동**: "지금 키 하나 리맵해보기" 인라인 데모.
...
### ③ Pro 페이월
- **Pro 기능 = ① 키보드 연결 시 프로필 자동 전환 + ② 다크모드/테마**(2026-05-31 추가). 평소 둘 다 `ProBadge` 달고 visible-but-locked.
- 트리거: 수동 프로파일 전환 N회 / 잠긴 자동전환 클릭 / **설정에서 다크모드 토글 시도** / 10일+ 사용. 나그 모달 ❌, 맥락 시점 ✅.
- 레이아웃: Raycast식 — 이득 3–5개 benefit-led, `PlanCard`(가격 대형), pill CTA. **Gumroad 브라우저 핸드오프 기대치 안내 + 브랜딩 일치**.
- **페이월 자체가 다크 프리미엄 = "Pro 다크 테마 미리보기"** 역할(라이트 Free 앱과 의도적 대비, #3 해소).
